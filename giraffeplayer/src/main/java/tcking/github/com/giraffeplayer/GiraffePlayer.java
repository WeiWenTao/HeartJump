...
public class GiraffePlayer {
    ...
    private void updatePausePlay() {
        if (videoView.isPlaying()) {
            $.id(R.id.app_video_play).image(R.drawable.icon_playing);
        } else {
            $.id(R.id.app_video_play).image(R.drawable.icon_stop);
        }
    }
...
//                        Toaster.show("download rate:" + extra);
    ...
    private void updateFullScreenButton() {
        if (getScreenOrientation() == ActivityInfo.SCREEN_ORIENTATION_LANDSCAPE) {
            $.id(R.id.app_video_fullscreen).image(R.drawable.icon_exit);
        } else {
            $.id(R.id.app_video_fullscreen).image(R.drawable.icon_full);
        }
    }
