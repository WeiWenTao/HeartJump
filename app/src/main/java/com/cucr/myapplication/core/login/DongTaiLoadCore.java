...
public class DongTaiLoadCore implements LoadByDongTai {
    ...
    //登录
    @Override
    public void login(Context context, String userName, String yzm, OnLoginListener loginListener) {
    ...
    private OnResponseListener<String> responseListener = new OnResponseListener<String>() {
        ...
        public void onSucceed(int what, Response<String> response) {
            if (what == REQUEST_YZM) {
                if (yzmListener != null && response.getHeaders().getResponseCode() == 200) {
                    yzmListener.onSuccess(response);
                }
            } else if (what == REQUEST_LOAD) {
                if (dongTaiListener != null && response.getHeaders().getResponseCode() == 200) {
